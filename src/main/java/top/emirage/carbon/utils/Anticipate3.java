...
//此处应该获得新的一年的碳排放量,获得的系数是 GDP  人口  能源强度  能源结构
        ...
        for(int i = 0;i<n;i++){
            ...
            n_carbon_e = n_carbon_e+UP_C;
            res[i] = new_carbon(coe,UP_GDP,UP_P,UP_E,UP_C,n_gdp,n_p,n_e_gdp,n_carbon_e);
        }
        return res;
    }
    /**
    ...
    public static double new_carbon(double[] coe,double UP_GDP ,double UP_P,double UP_E,double UP_C,
                                    double gdp, double p,double e_gdp,double carbon_e){
    ...
    public static double[] get_coefficient(double[] cs,double[] p,double[] e,double[] gdp,double[] carbon){
        int len = cs.length;
        double[] c_gdp = new double[len];//能源强度
        double[] c_p = new double[len];//人口
        double[] c_e_gdp = new double[len];//人均GDP
